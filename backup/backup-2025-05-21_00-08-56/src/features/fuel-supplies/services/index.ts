...
import type { FuelSupplyCreate, FuelSupplyUpdate, FuelSupply as ApiFuelSupply } from '@/core/api/types';
import type { 
  FuelSupply, 
...
function adaptApiResponseToFeatureType(apiSupply: ApiFuelSupply): FuelSupply {
  return {
    id: apiSupply.id,
    delivery_date: apiSupply.delivery_date,
    provider_id: apiSupply.supplier_id,
    tank_id: (apiSupply as any).tank_id || '',
    ...
    shift_id: (apiSupply as any).shift_id || '',
    payment_method: (apiSupply as any).payment_method || '',
    payment_status: (apiSupply as any).payment_status || '',
    created_at: apiSupply.created_at,
    updated_at: apiSupply.updated_at,
    provider: (apiSupply as any).provider,
    tank: (apiSupply as any).tank,
    employee: (apiSupply as any).employee
  };
}

// Helper function to adapt feature type to API request
function adaptFeatureTypeToApiRequest(featureSupply: CreateFuelSupplyRequest): FuelSupplyCreate {
  return {
...
export async function getFuelSupplies(filters?: FuelSupplyFilters) {
  ...
  if (filters) {
    ...
    if (filters.providerId) {
      supplies = supplies.filter(supply => supply.provider_id === filters.providerId);
    }
    ...
    if (filters.paymentStatus) {
      supplies = supplies.filter(supply => supply.payment_status === filters.paymentStatus);
    }
 ...
 */
export async function updateFuelSupply(id: string, data: UpdateFuelSupplyRequest) {
  ...
  if (data.quantity_liters !== undefined) apiData.quantity = data.quantity_liters;
  if (data.price_per_liter !== undefined) apiData.unit_price = data.price_per_liter;
