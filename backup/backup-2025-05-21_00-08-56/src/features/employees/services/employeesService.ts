...
import type { Employee, EmployeeFormData, EmployeeFilters, EmployeeSummary } from '../types/employees.types';
...
export const employeesService = {
  ...
  async createEmployee(employeeData: EmployeeFormData): Promise<Employee | null> {
  ...
  async updateEmployee(id: string, employeeData: EmployeeFormData): Promise<Employee | null> {
    // Convert from domain model to database format
    const dbEmployee = mapEmployeeToDb(employeeData);
  ...
  async getEmployeeSummary(): Promise<EmployeeSummary | null> {
    ...
    const summary: EmployeeSummary = {
      ...
      activeEmployees: mappedEmployees.filter(e => e.status === 'active').length,
      onLeaveEmployees: mappedEmployees.filter(e => e.status === 'on_leave').length,
      departments: Object.entries(
        mappedEmployees.reduce((acc, emp) => {
