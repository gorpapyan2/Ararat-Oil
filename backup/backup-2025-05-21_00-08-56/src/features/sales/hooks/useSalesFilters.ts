...
export function useSalesFilters({ defaultFilters = {} }: UseSalesFiltersProps = {}) {
  const { t } = useTranslation(['common', 'sales']);
  const [searchParams, setSearchParams] = useSearchParams();
  
  // Initialize filters from URL params or defaults
  const initialFilters: SalesFilters = useMemo(() => {
    const searchTerm = searchParams.get('search') || defaultFilters.searchTerm || '';
    const fuelType = (searchParams.get('fuelType') || defaultFilters.fuelType || 'all') as FuelTypeCode | 'all';
    const paymentStatus = (searchParams.get('paymentStatus') || defaultFilters.paymentStatus || 'all') as PaymentStatus | 'all';
    const fillingSystem = searchParams.get('fillingSystem') || defaultFilters.fillingSystem || 'all';
  ...
  const updateFilters = useCallback((newFilters: Partial<SalesFilters>) => {
    ...
    if (updatedFilters.paymentStatus && updatedFilters.paymentStatus !== 'all') {
      params.set('paymentStatus', updatedFilters.paymentStatus);
    }
    if (updatedFilters.fillingSystem && updatedFilters.fillingSystem !== 'all') {
      params.set('fillingSystem', updatedFilters.fillingSystem);
    }
    
    // Handle date range
    if (updatedFilters.dateRange?.from) {
      params.set('startDate', updatedFilters.dateRange.from.toISOString().split('T')[0]);
      
      if (updatedFilters.dateRange.to) {
        params.set('endDate', updatedFilters.dateRange.to.toISOString().split('T')[0]);
      }
    }
    
    // Handle min/max quantity
    if (updatedFilters.minQuantity) params.set('minQuantity', updatedFilters.minQuantity.toString());
    if (updatedFilters.maxQuantity) params.set('maxQuantity', updatedFilters.maxQuantity.toString());
    
    // Handle min/max amount
    if (updatedFilters.minAmount) params.set('minAmount', updatedFilters.minAmount.toString());
    if (updatedFilters.maxAmount) params.set('maxAmount', updatedFilters.maxAmount.toString());
