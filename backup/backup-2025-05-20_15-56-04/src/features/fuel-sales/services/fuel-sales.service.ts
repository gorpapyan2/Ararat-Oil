...
import { FuelSale, FuelSaleFormData, FuelSaleFilters, FuelSaleExportOptions } from "../types";
 ...
 */
export async function getFuelSales(filters?: FuelSaleFilters): Promise<FuelSale[]> {
  try {
    const apiFilters = {
    ...
    if (filters) {
      if (filters.fuel_type_id) {
        sales = sales.filter(sale => sale.fuel_type_id === filters.fuel_type_id);
      }
      
      if (filters.filling_system_id) {
        sales = sales.filter(sale => sale.filling_system_id === filters.filling_system_id);
      }
      
      if (filters.payment_method) {
        sales = sales.filter(sale => sale.payment_method === filters.payment_method);
      }
 ...
 */
export async function createFuelSale(saleData: FuelSaleFormData): Promise<FuelSale> {
  try {
 ...
 */
export async function updateFuelSale(id: string, saleData: Partial<FuelSaleFormData>): Promise<FuelSale> {
  try {
    // For partial updates, build a partial API request
    const apiData: Partial<ReturnType<typeof adaptFeatureTypeToApiRequest>> = {};
    
    if (saleData.filling_system_id !== undefined) apiData.filling_system_id = saleData.filling_system_id;
    if (saleData.fuel_type_id !== undefined) apiData.fuel_type_id = saleData.fuel_type_id;
    if (saleData.quantity !== undefined) apiData.quantity = saleData.quantity;
    if (saleData.price_per_liter !== undefined) apiData.price_per_liter = saleData.price_per_liter;
    if (saleData.total_price !== undefined) apiData.total_price = saleData.total_price;
    if (saleData.payment_method !== undefined) apiData.payment_method = saleData.payment_method;
    if (saleData.employee_id !== undefined) apiData.employee_id = saleData.employee_id;
 ...
 */
export async function getLatestFuelSale(fillingSystemId: string): Promise<FuelSale | null> {
  try {
 ...
 * Export sales data
 */
export async function exportFuelSales(options: FuelSaleExportOptions): Promise<string> {
  try {
