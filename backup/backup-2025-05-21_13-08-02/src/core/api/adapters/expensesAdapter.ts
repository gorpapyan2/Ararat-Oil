 ...
 */
export function adaptApiExpenseToAppExpense(apiExpense: ApiExpense): AppExpense {
  ...
  const mappedCategory = categoryMap[apiExpense.category.toLowerCase()] || 'other';
  ...
  return {
    ...
    payment_method: (apiExpense.payment_status === 'paid' ? 'cash' : undefined) as PaymentMethod | undefined,
 ...
 */
export function adaptApiExpensesToAppExpenses(apiExpenses: ApiExpense[]): AppExpense[] {
 ...
 */
export function adaptAppExpenseToApiExpense(appExpense: AppExpense): Omit<ApiExpense, 'created_at' | 'updated_at'> {
  ...
  const paymentStatusMap: Record<PaymentStatus, 'paid' | 'pending' | 'cancelled'> = {
    'pending': 'pending',
    'completed': 'paid',
    'failed': 'cancelled',
    'refunded': 'cancelled'
 ...
 */
export function adaptAppExpensesToApiExpenses(appExpenses: AppExpense[]): Omit<ApiExpense, 'created_at' | 'updated_at'>[] {
