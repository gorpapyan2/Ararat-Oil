      ...
      <div className="flex gap-2">
        <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className={cn(
                "w-full justify-start text-left font-normal bg-[var(--color-50)] bg-opacity-$2 border-[var(--color-50)] border-opacity-$2 hover:bg-[var(--color-80)] bg-opacity-$2 transition-colors",
                !startDate && !endDate && "text-[var(--color-muted-foreground)]",
              )}
            >
              <CalendarIcon className="mr-2 h-4 w-4 text-[var(--color-muted-foreground)]" />
              {startDate || endDate
                ? formatDateRange()
          ...
          <PopoverContent
            className="w-auto p-0 border border-[var(--color-40)] border-opacity-$2 shadow-md"
            align="start"
          >
            <Calendar
              mode="range"
              ...
              className="p-3"
            />
            <div className="flex items-center justify-between p-3 border-t border-[var(--color-30)] border-opacity-$2 bg-[var(--color-30)] bg-opacity-$2">
              <Button
                variant="ghost"
                size="sm"
              ...
              <Button
                ...
                className="bg-[var(--color-90)] bg-opacity-$2 hover:bg-[var(--color-primary)] transition-colors"
              >
                {t("common.apply")}
              </Button>
            </div>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className="px-2 bg-[var(--color-50)] bg-opacity-$2 border-[var(--color-50)] border-opacity-$2 hover:bg-[var(--color-80)] bg-opacity-$2 transition-colors"
            >
          ...
          <DropdownMenuContent
            align="end"
            className="border border-[var(--color-40)] border-opacity-$2 shadow-md"
          >
            {presets.map((preset) => (
              <DropdownMenuItem
                key={preset.name}
                ...
                className="flex items-center gap-2 hover:bg-[var(--color-10)] bg-opacity-$2 transition-colors"
              >
                {preset.name}
              </DropdownMenuItem>
            ))}
            <DropdownMenuSeparator className="bg-[var(--color-30)] bg-opacity-$2" />
            <DropdownMenuItem
              onClick={clearDateRange}
              className="text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)] hover:bg-[var(--color-10)] bg-opacity-$2 transition-colors"
            >
