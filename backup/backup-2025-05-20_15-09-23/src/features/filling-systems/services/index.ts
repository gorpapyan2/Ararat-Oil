...
import type { FillingSystem as ApiFillingSystem, FillingSystemCreate, FillingSystemUpdate } from '@/core/api/types';
import type { 
  FillingSystem, 
...
function adaptApiResponseToFeatureType(apiFillingSystem: ApiFillingSystem): FillingSystem {
  return {
    ...
    tank_id: apiFillingSystem.tank_ids && apiFillingSystem.tank_ids.length > 0 
...
function adaptFeatureTypeToApiCreateRequest(featureFillingSystem: CreateFillingSystemRequest): FillingSystemCreate {
  return {
...
function adaptFeatureTypeToApiUpdateRequest(featureFillingSystem: UpdateFillingSystemRequest): FillingSystemUpdate {
 ...
 */
export async function getFillingSystemsWithFilters(filters?: FillingSystemFilters): Promise<FillingSystem[]> {
  ...
  if (filters) {
    if (filters.status) {
      fillingSystems = fillingSystems.filter(fs => fs.status === filters.status);
    }
    
    if (filters.tank_id) {
      fillingSystems = fillingSystems.filter(fs => fs.tank_id === filters.tank_id);
    }
    
    if (filters.search) {
      const searchLower = filters.search.toLowerCase();
      fillingSystems = fillingSystems.filter(fs => 
