...
import { getFuelSales, fuelSalesService as originalService } from './fuel-sales.service';

/**
 * Get fuel sales summary statistics
 */
export async function getFuelSalesSummary(dateRange?: { start: string; end: string }): Promise<FuelSaleSummary> {
  try {
    ...
    const salesByFuelType: Record<string, { quantity: number; amount: number }> = {};
    sales.forEach(sale => {
    ...
    sales.forEach(sale => {
      ...
      salesByPaymentMethod[method] = (salesByPaymentMethod[method] || 0) + sale.total_price;
    });
    ...
    const dailySales = Array.from(salesByDate.entries()).map(([date, stats]) => ({
      date,
      amount: stats.amount,
