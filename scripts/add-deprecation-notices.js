...
/**
 * This script adds deprecation notices to components that have been migrated to the feature-based architecture.
 * 
 * Usage:
 * node scripts/add-deprecation-notices.js
 * 
 * This will parse the component-deprecation-schedule.md file and add deprecation notices to all components listed there.
 */
...
`;
  ...
  function createBridgeComponent(component) {
    try {
      ...
      // Import the component to get its type
      const replacementImportName = component.name;
      const bridgeContent = `/**
 * @deprecated This component is deprecated and will be removed in the next major version.
 * Please use the standardized version from the feature directory instead:
 * {@link import('${component.replacement}')}
...
import { ${component.name} as Feature${component.name} } from "${component.replacement}";
 ...
 * {@link import('${component.replacement}')}
 */
export function ${component.name}(props: React.ComponentProps<typeof Feature${component.name}>) {
  // Issue a deprecation warning
  useEffect(() => {
    console.warn(
      '[Deprecation Warning] ${component.name} in ${component.path} is deprecated. ' +
...
`;
      ...
      console.error(`Error creating bridge for component ${component.name}:`, err);
      return false;
    }
  }
  ...
  async function main() {
    try {
      ...
      console.log('About to ask for option selection...');
      
      // Ask whether to add notices or create bridge components
      rl.question('Do you want to (1) add deprecation notices or (2) create bridge components? ', async (answer) => {
        ...
        if (answer === '1') {
          ...
          for (const component of components) {
            try {
              const result = await createBridgeComponent(component);
              if (result) {
                console.log(`✅ Created bridge component for ${component.name}`);
              } else {
                console.log(`⚠️ Skipped ${component.name}`);
              }
            } catch (error) {
