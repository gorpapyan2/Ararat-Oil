...
export function adaptApiSaleToAppSale(apiSale: ApiSale): AppSale {
  ...
  const fuelType: FuelTypeCode = fuelTypeMap[apiSale.fuel_type_id]
 ...
 */
export function adaptAppSaleToApiSale(
  appSale: AppSale
): Omit<ApiSale, "created_at" | "updated_at"> {
  // Map payment status to payment method
  const paymentMethod: string =
    appSale.payment_status === "pending" ? "pending" : "cash";
 ...
 */
export function adaptAppSalesToApiSales(
  appSales: AppSale[]
): Omit<ApiSale, "created_at" | "updated_at">[] {
