 ...
 */
export function adaptApiEmployeesToAppEmployees(apiEmployees: ApiEmployee[]): AppEmployee[] {
 ...
 */
export function adaptAppEmployeeToApiEmployee(appEmployee: AppEmployee): Omit<ApiEmployee, 'created_at' | 'updated_at'> {
  // Map the app's EmployeeStatus enum to the API status string
  let status: string = "active";
  
  if (appEmployee.status === "on_leave") {
    status = "on_leave";
  } else if (appEmployee.status === "terminated") {
    status = "inactive";
  }
 ...
 */
export function adaptAppEmployeesToApiEmployees(appEmployees: AppEmployee[]): Omit<ApiEmployee, 'created_at' | 'updated_at'>[] {
