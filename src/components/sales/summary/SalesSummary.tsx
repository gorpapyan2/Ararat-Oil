...
export function SalesSummary({ sales }: SalesSummaryProps) {
  ...
  const avgPricePerUnit = totalLiters > 0 
    ? totalSales / totalLiters 
    : 0;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">
            {avgPricePerUnit.toLocaleString(undefined, { maximumFractionDigits: 0 })} ֏
          </div>
          <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Sales by Fuel Type</CardTitle>
        </CardHeader>
        <CardContent>
