...
export function ColorContrastChecker({
  ...
  const { ratio, passesAA } = checkColorContrast(foreground, background, isLargeText);
  ...
  return (
    <div className="p-4 border rounded-md space-y-4">
      ...
      >
        <p className={isLargeText ? "text-2xl" : "text-base"}>
          Sample Text
        </p>
      </div>
      ...
      <div className="bg-muted p-4 rounded-md">
        <div className="flex justify-between items-center">
          ...
          <div 
            className={`px-3 py-1 rounded-full ${
              passesAA ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
            }`}
            role="status"
          >
        ...
        <div className="mt-2 text-sm">
          <p>Required ratio for {isLargeText ? "large" : "normal"} text: {isLargeText ? "3" : "4.5"}:1</p>
        </div>
      </div>
    </div>
  );
