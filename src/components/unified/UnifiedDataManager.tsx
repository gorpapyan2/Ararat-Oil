...
export function UnifiedDataManager() {
  ...
  const { data: providersData = [], isLoading: providersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((provider) => ({
        id: provider.id || "",
        name:
          provider.name || `Provider ${provider.id?.slice(0, 4) || "Unknown"}`,
      }));
    },
  });
  ...
  const filteredSupplies = useMemo(() => {
    ...
    if (filters.tankId && filters.tankId !== "all") {
      filtered = filtered.filter(
        (supply) => supply.tank && supply.tank.id === filters.tankId,
      );
    }

    if (filters.provider && filters.provider !== "all") {
      filtered = filtered.filter(
        (supply) => supply.provider_id === filters.provider,
      );
    }
  ...
  const filteredSales = useMemo(() => {
    ...
    if (filters.systemId && filters.systemId !== "all") {
      filtered = filtered.filter(
        (sale) => sale.filling_system_id === filters.systemId,
      );
    }

    if (filters.salesFuelType && filters.salesFuelType !== "all") {
      filtered = filtered.filter(
        (sale) => sale.fuel_type === filters.salesFuelType,
      );
    }
  ...
  const filteredExpenses = useMemo(() => {
    ...
    if (filters.expenseCategory && filters.expenseCategory !== "all") {
      filtered = filtered.filter(
        (expense) => expense.category === filters.expenseCategory,
      );
    }
  ...
  const suppliesColumns: ColumnDef<FuelSupply>[] = [
    ...
    },
    {
      id: "tank",
      header: () => (
        <div className="text-left font-medium">{t("fuelSupplies.tank")}</div>
      ),
  ...
  const salesColumns: ColumnDef<Sale>[] = [
    {
      id: "date",
      header: () => (
        <div className="text-left font-medium">{t("common.date")}</div>
      ),
      accessorKey: "date",
      cell: ({ row }) => {
        const saleDate = row.getValue("date") as string;
        return (
          <div className="font-medium">{format(new Date(saleDate), "PP")}</div>
        );
      },
    },
    {
      ...
      header: () => (
        <div className="text-left font-medium">{t("sales.fillingSystem")}</div>
      ),
    ...
    },
    {
      id: "quantity",
      header: () => (
        <div className="text-right font-medium">{t("common.quantity")}</div>
      ),
    ...
    },
    {
      id: "price_per_unit",
      header: () => (
        <div className="text-right font-medium">{t("sales.unitPrice")}</div>
      ),
    ...
    },
    {
      id: "total_sales",
      header: () => (
        <div className="text-right font-medium">{t("sales.totalSales")}</div>
      ),
  ...
  const expensesColumns: ColumnDef<Expense>[] = [
    {
      id: "date",
      header: () => (
        <div className="text-left font-medium">{t("expenses.expenseDate")}</div>
      ),
      accessorKey: "date",
      cell: ({ row }) => {
    ...
    },
    {
      id: "amount",
      header: () => (
        <div className="text-right font-medium">{t("expenses.amount")}</div>
      ),
      accessorKey: "amount",
      cell: ({ row }) => {
    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      >
        <TabsList className="mb-4">
          <TabsTrigger value="fuel-supplies">
            {t("unifiedData.fuelSuppliesTab")}
          </TabsTrigger>
          <TabsTrigger value="sales">{t("unifiedData.salesTab")}</TabsTrigger>
          <TabsTrigger value="expenses">
            {t("unifiedData.expensesTab")}
          </TabsTrigger>
        </TabsList>
