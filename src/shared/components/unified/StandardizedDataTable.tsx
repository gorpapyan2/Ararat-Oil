...
export function StandardizedDataTable<TData extends object>({
    ...
    if (onEdit || onDelete) {
      cols.push({
        ...
        cell: ({ row }: { row: { original: TData } }) => {
          ...
          const id = (rowData as TData & { id: string | number }).id;
  ...
  const dataTableExportOptions: ExportOptions<TData> | undefined =
  ...
  useEffect(() => {
    if (
      ...
      keyboardNavigation.focusOnLoad &&
      safeData.length > 0
    ) {
      setFocusedRowIndex(0);
    }
