 ...
 */
export function useEntityDialog<T = any>(options?: EntityDialogOptions<T>) {
   ...
   */
  const handleCreateSuccess = useCallback((createdEntity: T) => {
   ...
   */
  const handleUpdateSuccess = useCallback((updatedEntity: T) => {
   ...
   */
  const handleDeleteSuccess = useCallback((entityId: string | number) => {
