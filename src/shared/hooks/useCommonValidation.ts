...
export function useCommonValidation() {
  ...
  const phoneNumber = useCallback(() => {
    return z
      .string()
      .regex(REGEX_PATTERNS.PHONE, t("validation.invalidPhoneNumber"))
      .optional();
  }, [t]);
  ...
  const number = useCallback(
    (options?: {
      ...
      if (min !== undefined) {
        schema = schema.min(
          min,
          t("validation.minNumber", { field: name, min })
        );
      }
      ...
      if (max !== undefined) {
        schema = schema.max(
          max,
          t("validation.maxNumber", { field: name, max })
        );
      }
  ...
  const date = useCallback(
    (options?: {
      ...
      if (past) {
        schema = schema.max(
          new Date(),
          t("validation.dateMustBePast", { field: name })
        );
      }
      ...
      if (future) {
        schema = schema.min(
          new Date(),
          t("validation.dateMustBeFuture", { field: name })
        );
      }
   ...
   */
  const conditionalField = useCallback(
    <T extends z.ZodType>(
      schema: T,
