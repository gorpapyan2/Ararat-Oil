...
export function useCommonValidation() {
   ...
   */
  const number = useCallback((options?: { min?: number; max?: number; fieldName?: string }) => {
    const name = options?.fieldName ? t(`fields.${options.fieldName}`) : t('common.field');
    let schema = z.number({
      required_error: t('validation.required', { field: name }),
    ...
    if (options?.min !== undefined) {
      schema = schema.min(options.min, t('validation.minValue', { field: name, value: options.min }));
    }
    
    if (options?.max !== undefined) {
      schema = schema.max(options.max, t('validation.maxValue', { field: name, value: options.max }));
    }
   ...
   */
  const date = useCallback((options?: { min?: Date; max?: Date; fieldName?: string }) => {
    const name = options?.fieldName ? t(`fields.${options.fieldName}`) : t('common.field');
    let schema = z.date({
      required_error: t('validation.required', { field: name }),
    ...
    if (options?.min) {
      schema = schema.min(options.min, t('validation.minDate', { field: name, date: options.min.toLocaleDateString() }));
    }
    
    if (options?.max) {
      schema = schema.max(options.max, t('validation.maxDate', { field: name, date: options.max.toLocaleDateString() }));
    }
   ...
   */
  const matchField = useCallback((field: string, message?: string) => {
   ...
   */
  const enumValue = useCallback(<T extends [string, ...string[]]>(values: T, fieldName?: string) => {
    const name = fieldName ? t(`fields.${fieldName}`) : t('common.field');
