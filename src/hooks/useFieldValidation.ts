...
export function useFieldValidation() {
   ...
   */
  const creditCardSchema = useCallback((required: boolean = true) => {
   ...
   */
  const expirationDateSchema = useCallback((required: boolean = true) => {
    const schema = z.string()
      .regex(/^(0[1-9]|1[0-2])\/([0-9]{2})$/, t('validation.expirationDate.format'))
      .refine(
        (expDate) => {
   ...
   * CVV Code validation
   */
  const cvvSchema = useCallback((required: boolean = true) => {
    const schema = z.string()
   ...
   */
  const urlSchema = useCallback((requireHttps: boolean = false) => {
    // Base URL schema
   ...
   */
  const postalCodeSchema = useCallback((country: string = 'US') => {
   ...
   */
  const idNumberSchema = useCallback((type: 'ssn' | 'taxId' | 'custom' = 'ssn', customPattern?: RegExp, customMessage?: string) => {
    const patterns: Record<string, { pattern: RegExp; message: string }> = {
      ssn: { 
        pattern: /^\d{3}-\d{2}-\d{4}$/, 
