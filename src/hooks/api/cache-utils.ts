 ...
 */
export function invalidateQueries(queryClient: QueryClient, queryKeys: QueryKey[]): void {
...
export function createQueryKeys(resourceName: string) {
  return {
    ...
    list: (filters: any) => [...createQueryKeys(resourceName).lists(), filters] as const,
    details: () => [...createQueryKeys(resourceName).all, 'detail'] as const,
    detail: (id: string) => [...createQueryKeys(resourceName).details(), id] as const,
