...
// Mock API for fuel management
const fuelManagementApi = {
  getFuelManagementSummary: async (filters?: FuelFilters) => {
    // Mock implementation
    return {
      data: {
        tanks: {
          totalVolume: 50000,
          availableVolume: 35000,
          utilizationRate: 70,
          list: [],
          byType: {},
        },
...
export default function FuelManagementDashboard() {
  ...
  const [selectedFuelType, setSelectedFuelType] =
    useState<FuelTypeSelection>("all");
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const response =
        await fuelManagementApi.getFuelManagementSummary(filters);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card className="relative overflow-hidden hover:shadow-md transition-shadow">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">
              {t("fuelManagement.addFuelEntryDesc")}
            </CardDescription>
          </CardHeader>
          <CardFooter>
            <Button
              className="w-full"
              onClick={() => navigate("fuel-supplies")}
            >
        ...
        <Card className="relative overflow-hidden hover:shadow-md transition-shadow">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">
              {t("fuelManagement.startInventoryDesc")}
            </CardDescription>
          </CardHeader>
          <CardFooter>
            <Button
        ...
        <Card className="relative overflow-hidden hover:shadow-md transition-shadow">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-xs">
              {t("fuelManagement.generateReportDesc")}
            </CardDescription>
          </CardHeader>
          <CardFooter>
            <Button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatNumber(summary.tanks.totalVolume)} L
            </div>
            <p className="text-xs text-muted-foreground">
              {t("fuelManagement.availableVolume", {
                volume: formatNumber(summary.tanks.availableVolume),
              })}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatNumber(summary.supplies.total)}
            </div>
            <p className="text-xs text-muted-foreground">
              {t("fuelManagement.totalCost", {
                amount: formatCurrency(summary.supplies.totalCost),
              })}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatNumber(summary.tanks.utilizationRate)}%
            </div>
            <Progress
              value={summary.tanks.utilizationRate}
              className="h-2 mt-2"
            />
          </CardContent>
        </Card>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {t("fuelManagement.consumptionTrendDesc")}
                  </CardDescription>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">
                    {t("fuelManagement.selectTank")}
                  </label>
                  <Select value={selectedTank} onValueChange={setSelectedTank}>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("fuelManagement.selectTank")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">
                    {t("fuelManagement.selectFuelType")}
                  </label>
                  <Select
                    value={selectedFuelType}
                    onValueChange={(value) =>
                      setSelectedFuelType(value as FuelTypeSelection)
                    }
                  >
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("fuelManagement.selectFuelType")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">{t("common.all")}</SelectItem>
                      {Object.entries(summary.tanks.byType).map(
              ...
              <Tabs defaultValue="supplies">
                <TabsList className="w-full">
                  <TabsTrigger value="supplies" className="flex-1">
                    {t("fuelManagement.supplies")}
                  </TabsTrigger>
                  <TabsTrigger value="levels" className="flex-1">
                    {t("fuelManagement.tankLevels")}
                  </TabsTrigger>
                </TabsList>
                <TabsContent value="supplies">
                  <ScrollArea className="h-[300px]">
                    <div className="space-y-4">
                      {(summary.supplies.list || []).map(
                        (supply: {
                          id: string;
                          fuel_type: string;
                          quantity: number;
                          ...
                          >
                            ...
                            <div className="text-right">
                              <div className="font-medium">
                                {formatNumber(supply.quantity)} L
                              </div>
                              <div className="text-sm text-muted-foreground">
                        ...
                        (tank: {
                          ...
                          >
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <Badge
                                variant={
                                  "status" in tank && tank.status === "active"
                                    ? "default"
                                    : "secondary"
                                }
                              >
                                {t(
                                  "status." +
                                    ("status" in tank ? tank.status : "unknown")
                                )}
                              </Badge>
                            </div>
                            <div className="space-y-2">
                              <div className="flex justify-between text-sm">
                                ...
                                <span>
                                  {formatNumber(tank.current_level)} L
                                </span>
                              </div>
                              <Progress
                                value={
