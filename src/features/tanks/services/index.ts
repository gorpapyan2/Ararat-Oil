...
export async function adjustTankLevel(
  tankId: string, 
  adjustment: { change_amount: number; change_type: 'add' | 'subtract'; reason?: string }
  ...
  const tankLevelChange: TankLevelChange = {
    id: response.data.id || '',
    tank_id: tankId,
    previous_level: response.data.current_level - (adjustment.change_type === 'add' ? adjustment.change_amount : -adjustment.change_amount),
