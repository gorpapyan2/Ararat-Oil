...
export function IncomeExpenseOverview() {
  ...
  const chartData = useMemo<ChartDataItem[]>(() => {
    ...
    const salesByMonth = months.map(
      ({ month, monthLabel, startOfMonth, endOfMonth }) => {
    ...
    const profitGrowth =
      chartData.length >= 2
        ? ((chartData[chartData.length - 1].profit -
            chartData[chartData.length - 2].profit) /
            Math.abs(chartData[chartData.length - 2].profit || 1)) *
          100
        : 0;
    ...
    const incomeChange =
      chartData.length >= 2
        ? ((chartData[chartData.length - 1].income -
            chartData[chartData.length - 2].income) /
            Math.abs(chartData[chartData.length - 2].income || 1)) *
          100
        : 0;

    const expenseChange =
      chartData.length >= 2
        ? ((chartData[chartData.length - 1].expense -
            chartData[chartData.length - 2].expense) /
            Math.abs(chartData[chartData.length - 2].expense || 1)) *
          100
        : 0;
  ...
  const CustomTooltip = ({ active, payload, label }: CustomTooltipProps) => {
    if (active && payload && payload.length) {
      const income =
        payload.find((p: TooltipPayloadItem) => p.dataKey === "income")?.value || 0;
      const expense =
        payload.find((p: TooltipPayloadItem) => p.dataKey === "expense")?.value || 0;
      const profit = income - expense;
        ...
        <div className="rounded-lg border bg-gray-50 p-4 shadow-lg animate-in fade-in-50 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95">
          ...
          <div className="mt-3 pt-3 border-t border-muted">
            <div className="flex justify-between items-center text-sm">
              ...
              <span
                className={`font-semibold ${isProfit ? "text-emerald-600" : "text-rose-600"}`}
              >
    ...
    return (
      ...
      >
        <div className="flex justify-between items-start">
          ...
          <span
            className={`rounded-full p-1.5 ${color.replace("text-", "bg-")}/20`}
          >
            {icon}
          </span>
        </div>
        <div className="mt-1">
          <div className="text-2xl font-bold tracking-tight">
            {prefix && (
              <span className="text-lg font-normal mr-0.5">{prefix}</span>
            )}
      ...
      <CardHeader className="space-y-1">
                ...
                {isLoading ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart
                      ...
                      onMouseMove={(data: ChartEventData) => {
                        if (data.activeTooltipIndex !== undefined) {
                          handleMouseEnter(data, data.activeTooltipIndex);
                        }
                      }}
                      onMouseLeave={handleMouseLeave}
                      onClick={(data: ChartEventData) => {
                        if (data.activeTooltipIndex !== undefined) {
                          setActiveIndex(
        ...
        <CardDescription className="flex flex-col sm:flex-row justify-between gap-2 items-start sm:items-center">
          ...
          <span className="flex items-center gap-2 text-sm">
            <span>{t("dashboard.netProfit")}:</span>
            <span
              className={`font-semibold flex items-center ${totals.profit >= 0 ? "text-emerald-600" : "text-rose-600"}`}
            >
      ...
      <CardFooter className="border-t px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div className="flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>
            {chartType === "monthly"
              ? safeT(
                  "dashboard.clickBarForDetails",
                  "Click on a bar for more details"
                )
              : safeT(
                  "dashboard.switchToMonthlyView",
          ...
          <TooltipProvider>
            <TooltipUI>
              ...
              <TooltipContent>
                <p className="text-xs">
                  {safeT(
                    "dashboard.profitTooltip",
                    "Net profit calculated as income minus expenses"
                  )}
                </p>
              </TooltipContent>
            </TooltipUI>
