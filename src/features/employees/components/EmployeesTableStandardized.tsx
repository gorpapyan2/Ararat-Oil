...
export function EmployeesTableStandardized({
    ...
    },
    {
      ...
      cell: (value: string) => (
        <div className="flex items-center gap-2" role="cell">
          ...
          <span>{value ? format(new Date(value), 'PP') : t('common:notAvailable')}</span>
          {value && (
            <VisuallyHidden>
    ...
    },
    {
      ...
      cell: (value: number) => (
        <div className="flex items-center gap-2 justify-end" role="cell">
          ...
          {value && (
            <VisuallyHidden>
              {t('employees:aria.salary', '{{amount}} Armenian Dram', { amount: value.toLocaleString() })}
            </VisuallyHidden>
          )}
        </div>
      ),
  ...
  const getAccessibleRowData = useCallback((employee: Employee) => {
    return {
      name: formatEmployeeName(employee),
      position: employee.position || t('common:notAvailable'),
      department: employee.department || t('common:notAvailable'),
      hireDate: employee.hire_date ? format(new Date(employee.hire_date), 'PP') : t('common:notAvailable'),
      email: employee.email || t('common:notAvailable'),
      phone: employee.phone || t('common:notAvailable'),
      salary: employee.salary ? `${employee.salary.toLocaleString()} ֏` : t('common:notAvailable'),
  ...
  return (
    <ResponsiveDataTable
      ...
      aria-label={t('employees:aria.employeesTable', 'Employees table with sortable columns')}
      getRowAriaLabel={(employee) => {
        const data = getAccessibleRowData(employee);
        return t('employees:aria.employeeRow', 
      ...
      keyboardNavigation={{
        ...
        onKeyDown: (e, row) => {
          ...
          if (e.key === 'Delete' && onDelete && window.confirm(t('employees:confirmDelete', 'Are you sure you want to delete this employee?'))) {
            onDelete(row.id);
          }
        }
      }}
