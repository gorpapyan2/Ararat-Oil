...
export const fetchFuelTanks = async (): Promise<FuelTank[]> => {
  ...
  return (data || []).map((item) => ({
    ...
    current_level:
      typeof item.current_level === "number" ? item.current_level : 0,
  }));
};

export const createFuelTank = async (
  ...
  return {
    ...
    current_level:
      typeof data.current_level === "number" ? data.current_level : 0,
  };
};
...
export const updateTankLevel = async (
  ...
  return {
    ...
    current_level:
      typeof tankData.current_level === "number" ? tankData.current_level : 0,
  };
};
